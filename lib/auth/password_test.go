...
func (s *PasswordSuite) TestChangePasswordWithToken(c *C) {
	...
	resetToken, err := s.a.CreateUserToken(CreateUserTokenRequest{
		Name: username,
	})
	c.Assert(err, IsNil)
...
func (s *PasswordSuite) TestChangePasswordWithTokenOTP(c *C) {
	...
	userToken, err := s.a.CreateUserToken(CreateUserTokenRequest{
		Name: username,
	})

	otpToken, err := totp.GenerateCode(userToken.GetOTPKey(), s.bk.Clock().Now())
	c.Assert(err, IsNil)
